...
SCRIPT0="python -u benchmark_raw_collectives/all_to_allv_highly_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST $PROFILE \
        --library pccl"

SCRIPT1="python -u benchmark_raw_collectives/all_to_allv_highly_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST \
        --pccl-disable-cpp-backend \
        --library mpi"
        
SCRIPT2="python -u benchmark_raw_collectives/all_to_allv_highly_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST \
        --pccl-disable-cpp-backend \
        --library nccl"
