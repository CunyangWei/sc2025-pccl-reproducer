...
class NCCLCommunicator:
    ...
    def initialize_comm(
        ...
        # Debug: Check if communicator was properly initialized
        if self.comm.value is None or self.comm.value == 0:
            raise RuntimeError(f"NCCL communicator initialization failed: comm.value = {self.comm.value}")
        # Validate the communicator by calling ncclCommUserRank
        test_rank = ctypes.c_int()
        ret = CommHandler.get_comm_lib().ncclCommUserRank(self.comm, ctypes.byref(test_rank))
        self.check_nccl_error(ret, "Failed to validate NCCL communicator")
    ...
    def my_all_to_all(self, output_tensor, input_tensor, stream=None):
        """
        Performs all-to-all communication using NCCL grouped operations.
        Each process sends data to every other process and receives data from every other process.
        
        Args:
            output_tensor: Output tensor where results will be stored
            input_tensor: Input tensor containing data to send
            stream: CUDA stream to use (optional)
        """
        if not input_tensor.is_cuda or not output_tensor.is_cuda:
            raise ValueError("Both input and output tensors must be on CUDA/ROCm device")
        
        if input_tensor.numel() != output_tensor.numel():
            raise ValueError("Input and output tensors must have the same number of elements")

        # Get number of ranks in this communicator
        nranks = ctypes.c_int()
        ret = CommHandler.get_comm_lib().ncclCommCount(self.comm, ctypes.byref(nranks))
        self.check_nccl_error(ret, "ncclCommCount failed")
        nranks = nranks.value

        # Get current rank
        rank = ctypes.c_int()
        ret = CommHandler.get_comm_lib().ncclCommUserRank(self.comm, ctypes.byref(rank))
        self.check_nccl_error(ret, "ncclCommUserRank failed")
        rank = rank.value

        total_elems = input_tensor.numel()
        if total_elems % nranks != 0:
            raise ValueError(f"Input tensor size ({total_elems}) must be divisible by number of ranks ({nranks})")
        
        block_size = total_elems // nranks
        
        if stream is None:
            stream = torch.cuda.current_stream(input_tensor.device)
            
        # Use NCCL grouped operations for optimal performance
        ret = CommHandler.get_comm_lib().ncclGroupStart()
        self.check_nccl_error(ret, "ncclGroupStart failed")
        
        try:
            for r in range(nranks):
                # Calculate pointers for send and receive
                send_offset = r * block_size * input_tensor.element_size()
                recv_offset = r * block_size * output_tensor.element_size()
                
                send_ptr = input_tensor.data_ptr() + send_offset
                recv_ptr = output_tensor.data_ptr() + recv_offset
                
                # Send block r to rank r
                ret = CommHandler.get_comm_lib().ncclSend(
                    buffer_type(send_ptr),
                    ctypes.c_size_t(block_size * input_tensor.element_size()),
                    ctypes.c_int(0),  # ncclInt8 for byte-level transfer
                    ctypes.c_int(r),
                    self.comm,
                    ctypes.c_void_p(stream.cuda_stream),
                )
                self.check_nccl_error(ret, f"ncclSend to rank {r} failed")
                
                # Receive block from rank r
                ret = CommHandler.get_comm_lib().ncclRecv(
                    buffer_type(recv_ptr),
                    ctypes.c_size_t(block_size * output_tensor.element_size()),
                    ctypes.c_int(0),  # ncclInt8 for byte-level transfer
                    ctypes.c_int(r),
                    self.comm,
                    ctypes.c_void_p(stream.cuda_stream),
                )
                self.check_nccl_error(ret, f"ncclRecv from rank {r} failed")
        
        finally:
            # Always call ncclGroupEnd even if there's an error
            ret = CommHandler.get_comm_lib().ncclGroupEnd()
            self.check_nccl_error(ret, "ncclGroupEnd failed")
    
    def get_comm_handle(self):
        if self.comm is None:
            raise RuntimeError("NCCL communicator is None")
        handle = self.comm.value
        if handle is None or handle == 0:
            raise RuntimeError(f"NCCL communicator handle is invalid: {handle}")
        return handle 
