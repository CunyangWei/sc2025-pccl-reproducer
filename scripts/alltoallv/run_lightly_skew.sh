...
module load PrgEnv-gnu cray-mpich/8.1.33 craype-accel-nvidia80
module swap gcc-native gcc-native/13.2
...
SCRIPT0="python -u benchmark_raw_collectives/all_to_allv_light_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST $PROFILE \
        --library pccl"

SCRIPT3="python -u benchmark_raw_collectives/all_to_allv_light_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST $PROFILE \
        --library pccl \
        --pccl-algorithm simple_2D "

SCRIPT1="python -u benchmark_raw_collectives/all_to_allv_light_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST \
        --pccl-disable-cpp-backend \
        --library mpi"
        
SCRIPT2="python -u benchmark_raw_collectives/all_to_allv_light_skew.py \
        --num-gpus-per-node $GPUS_PER_NODE \
        --machine perlmutter $TEST \
        --pccl-disable-cpp-backend \
        --library nccl"
